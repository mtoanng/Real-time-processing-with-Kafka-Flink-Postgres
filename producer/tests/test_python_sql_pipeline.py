...
class PythonSqlPipelineTests(unittest.TestCase):
    def test_sql_preserves_two_branches_and_source_category_metric_grain(self) -> None:
        ddl = render_tables(config())
        inserts = "\n".join(insert_statements())
        self.assertIn("'value.format' = 'avro-confluent'", ddl)
        self.assertIn("'format' = 'debezium-json'", ddl)
        ...
        self.assertEqual(7, len(table_ddl(config())))
