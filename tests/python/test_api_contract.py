...
from taobao_events.contracts import parse_event
from taobao_events.kafka import event_to_avro, kafka_key
...
class FakeServing:
    ...
    def features(self, user_id: int) -> dict[str, object]:
        return {"user_id": user_id, "available": False, "features": None, "ttl_seconds": -2}
...
class ApiContractTests(unittest.TestCase):
    ...
    def test_http_boundary_owns_identity_when_client_omits_event_id(self) -> None:
        payload = self.direct.to_dict()
        payload.pop("event_id")
        response = self.client.post("/v1/events", json=payload)
        self.assertEqual(202, response.status_code)
        self.assertEqual(self.direct.event_id, response.json()["event_id"])
        self.assertEqual(self.direct, self.publisher.events[0])
    ...
    def test_serving_routes_return_logical_backing_state(self) -> None:
        ...
        self.assertEqual(
            {"user_id": 42, "available": False, "features": None, "ttl_seconds": -2},
            self.client.get("/v1/users/42/features").json(),
        )
