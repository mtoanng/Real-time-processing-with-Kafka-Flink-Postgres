...
-- Platform-owned SQL sink boundary. Flink SQL writes JSON contracts to Kafka;
-- ClickHouse consumes them without project-authored JVM sink code.
...
SELECT
    ...
    toUInt64(source_sequence) AS source_sequence,
    replay_run_id,
...
SELECT
    ...
    toUInt64(item_id) AS item_id,
    toUInt64(source_category_id) AS source_category_id,
    toUInt64(pv_count) AS pv_count,
    toUInt64(cart_count) AS cart_count,
    toUInt64(fav_count) AS fav_count,
    toUInt64(buy_count) AS buy_count,
...
SELECT
    ...
    category_id,
    behavior_type,
    event_time_ms AS event_time,
    replay_run_id,
    source_sequence,
    reason_code,
    reason_message,
    fromUnixTimestamp64Milli(observed_at_ms) AS observed_at,
    toUInt64(record_version) AS record_version
FROM taobao_behavior.stream_quality_events_queue;

-- Deprecated legacy Product CDC output retained for migration evidence only.
-- It is not populated or required by the active core profile.
