...
This repository intentionally uses the smallest professional CI/CD shape that
fits a single-host Data Engineering portfolio:
...
Append `taobao_deploy_key.pub` to the EC2 user's
...
| Variable | Example |
| --- | --- |
| `EC2_HOST` | `ec2-203-0-113-10.ap-southeast-1.compute.amazonaws.com` |
| `EC2_USER` | `ubuntu` |
...
For a short-lived portfolio demo, keep key authentication only, disable SSH
...
This is professional for a bounded, single-EC2 portfolio deployment. It is not
